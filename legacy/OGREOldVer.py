	...
	def run(self):
		...
		while True:
			if(remoteDirectory == ""):
				...
				print(myLogs.get())
				csvlogfile = open(r'C:\Users\ahankin\Documents\Radial\Tester2.csv', 'w', newline='')
				csvlogwriter = csv.writer(csvlogfile, delimiter=',',
								quotechar='|', quoting=csv.QUOTE_MINIMAL)
				#csvlogwriter.writerow(["pls","work"])
				...
				url = "Get-WmiObject CIM_DataFile -Computername Kopprdlenapp01 -Credential GSICCORP\\ahankin -filter `  \'Drive=\"C:\" and Path=\"" + remoteDirectory + "\" and Extension=\"log\"\' | Format-List *"
...
def remotegetlogs():
	...
	try:
		...
		#url = "Get-WmiObject CIM_DataFile -Computername Kopprdlenapp01 -Credential GSICCORP\\ahankin -filter `  \'Drive=\"C:\" and Path=\"" + newDir + "\" and Extension=\"log\"\' | Format-List *"
		#print(url)
		#p = subprocess.Popen(["powershell.exe",url], shell=True, stdout=subprocess.PIPE)
		#print(p.stdout.read())
		#output = p.communicate()[0].decode("utf-8")
